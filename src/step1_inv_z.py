...
from make_6_para_z import exec_gjf##計算した点のxyzfileを出す
...
def init_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E_init = pd.DataFrame(columns = ['a','b','theta','E','E_p','E_t1','E_t2','machine_type','status','file_name'])##隣接6分子
    else:
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    while True:
        ...
        for theta in [theta_init_prev-0.5,theta_init_prev,theta_init_prev+0.5]:
            ...
            df_val_theta = df_val[(df_val['a']==a)&(df_val['b']==b)&(df_val['theta']==theta)&(df_val['status']=='Done')]
            if len(df_val_theta)==0:
                return False,{'theta':theta}
            theta_list.append(theta);E_list.append(df_val_theta['E'].values[0])
